...
const loggerPath = path.join(dllDir, 'logger.dll');
//open({ library: 'spdlog', path: spdlogPath });
//open({ library: 'fmt', path: fmtPath });
open({ library: 'logger', path: loggerPath });

// 定义C++库函数接口
const LoggerLib = define({
    Logger_Initialize: {
        library: 'logger',
...
class Logger {
    ...
    initialize(fileName, policy, truncate = false, fileDir = '') {
        try {
            ...
            const result = LoggerLib.Logger_Initialize([fileName, policy, truncate, fileDir]);
            if (result) {
    ...
    debug(message) {
        try {
            if (this.initialized) {
                LoggerLib.Logger_Debug([message]);
            }
        } catch (error) {
    ...
    error(message) {
        try {
            if (this.initialized) {
                LoggerLib.Logger_Error([message]);
            }
        } catch (error) {
    ...
    critical(message) {
        try {
            if (this.initialized) {
                LoggerLib.Logger_Critical([message]);
            }
        } catch (error) {
    ...
    trace(message) {
        try {
            if (this.initialized) {
                LoggerLib.Logger_Trace([message]);
            }
        } catch (error) {
