...
#include "universal/mysql/field_mapper.hpp"

class FKUserMapper;
class FKUserEntity : public universal::mysql::BaseEntity {
    friend class FKUserMapper;
public:
    using FieldTypeList = universal::mysql::TypeList<
        std::uint32_t,                                       // id
        std::string,                                         // uuid
        std::string,                                         // username
        std::string,                                         // email
    ...
    std::vector<std::string> getFieldValues() const override {
        return {std::to_string(_id), _uuid, _username, _email, _password,
            universal::utils::time::time_point_to_str(_createTime),
            _updateTime ? universal::utils::time::time_point_to_str(*_updateTime) : "NULL"
        };
    }
private:
    std::uint32_t _id{0};
