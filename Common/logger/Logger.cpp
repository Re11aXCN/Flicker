...
std::string Logger::_createLogFlie(const std::string& fileDir, const std::string& fileName)
{
    ...
    {
    case SingleFile: {
        actualFilename = utils::concat(fileDir, utils::local_separator(), fileName, ".log.html");
        break;
    }
    case MultiplePerDay: {
        ...
        actualFilename = utils::concat(fileDir, utils::local_separator(), fileName, " - [", dateStr.str(), "].log.html");
        break;
    }
    case MultiplePerRun: {
        auto now = std::chrono::system_clock::now();
