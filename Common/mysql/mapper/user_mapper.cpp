...
UserMapper::UserMapper() : BaseMapper<UserEntity, std::uint32_t>() {

}

constexpr std::string UserMapper::getTableName() const {
    return "users";
}

constexpr std::string UserMapper::findByIdQuery() const {
...
constexpr std::string UserMapper::findAllQuery() const {
    return "SELECT id, uuid, username, email, password, "
           "UNIX_TIMESTAMP(create_time) * 1000 as create_time_ms, "
           "UNIX_TIMESTAMP(update_time) * 1000 as update_time_ms "
...
constexpr std::string UserMapper::findByEmailQuery() const {
    return "SELECT id, uuid, username, email, password, "
        "UNIX_TIMESTAMP(create_time) * 1000 as create_time_ms, "
        "UNIX_TIMESTAMP(update_time) * 1000 as update_time_ms "
...
constexpr std::string UserMapper::findByUsernameQuery() const {
    return "SELECT id, uuid, username, email, password, "
        "UNIX_TIMESTAMP(create_time) * 1000 as create_time_ms, "
        "UNIX_TIMESTAMP(update_time) * 1000 as update_time_ms "
...
constexpr std::string UserMapper::insertQuery() const {
    return "INSERT INTO " + getTableName() + 
           " (username, email, password) "
           "VALUES (?, ?, ?)";
}

constexpr std::string UserMapper::deleteByIdQuery() const {
    return "DELETE FROM " + getTableName() + " WHERE id = ?";
}

constexpr std::string UserMapper::updatePasswordByEmailQuery() const {
    return "UPDATE " + getTableName() + 
           " SET password = ?, update_time = ? WHERE email = ?";
}

constexpr std::string UserMapper::deleteByEmailQuery() const {
    return "DELETE FROM " + getTableName() + " WHERE email = ?";
}

constexpr std::string UserMapper::_isUsernameExistsQuery() const
{
    return "SELECT 1 FROM " + getTableName() + " WHERE username = ? LIMIT 1";
}


constexpr std::string UserMapper::_isEmailExistsQuery() const
{
    return "SELECT 1 FROM " + getTableName() + " WHERE email = ? LIMIT 1";
}


constexpr std::string UserMapper::_findPasswordByEmailQuery()
{
    return "SELECT password FROM " + getTableName() + " WHERE email = ?";
}


constexpr std::string UserMapper::_findPasswordByUsernameQuery()
{
    return "SELECT password FROM " + getTableName() + " WHERE username = ?";
}

void UserMapper::bindInsertParams(MySQLStmtPtr& stmtPtr, const UserEntity& entity) const {
...
UserEntity UserMapper::createEntityFromBinds(MYSQL_BIND* binds, MYSQL_FIELD* fields, unsigned long* lengths,
...
UserEntity UserMapper::createEntityFromRow(MYSQL_ROW row, MYSQL_FIELD* fields,
...
std::optional<UserEntity> UserMapper::findByEmail(const std::string& email) {
    try {
...
std::optional<UserEntity> UserMapper::findByUsername(const std::string& username) {
    try {
...
DbOperator::Status UserMapper::updatePasswordByEmail(const std::string& email, const std::string& password) 
{
    try {
        ...
        LOGGER_ERROR(std::format("updatePasswordByEmail error: {}", e.what()));
        return DbOperator::Status::DatabaseError;
    }
}

DbOperator::Status UserMapper::deleteByEmail(const std::string& email) 
{
    try {
        ...
        LOGGER_ERROR(std::format("deleteByEmail error: {}", e.what()));
        return DbOperator::Status::DatabaseError;
    }
}

bool UserMapper::isUsernameExists(const std::string& username)
{
    try {
        ...
        LOGGER_ERROR(std::format("isUsernameExists error: {}", e.what()));
        throw;
    }
}

bool UserMapper::isEmailExists(const std::string& email)
{
    try {
        ...
        LOGGER_ERROR(std::format("isEmailExists error: {}", e.what()));
        throw;
    }
}

std::optional<std::string> UserMapper::findUuidByEmail(const std::string& email)
{
    try {
        ...
        LOGGER_ERROR(std::format("findPasswordByEmail error: {}", e.what()));
        throw;
    }
}

std::optional<std::string> UserMapper::findUuidByUsername(const std::string& username)
{
    try {
        std::vector<std::string> fields = { "uuid" };
        ...
        LOGGER_ERROR(std::format("findPasswordByUsername error: {}", e.what()));
        throw;
    }
}

std::optional<std::string> UserMapper::findPasswordByEmail(const std::string& email)
{
    try {
...
std::optional<std::string> UserMapper::findPasswordByUsername(const std::string& username)
{
    try {
        std::vector<std::string> fields = { "password" };
        auto bindCondition = QueryConditionBuilder::eq_("username", varchar{ username.data(), (unsigned long)username.length() });
