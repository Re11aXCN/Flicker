...
{
    std::string query = utils::concat(findAllQuery(),
    ...
    try {
        ...
        std::string query = fieldNames.size() > 1
            ? utils::concat(
                ...
                " SET ", utils::join_range(" = ?, ", fieldNames),
                " = ? WHERE  id = ?")
            : utils::concat(
                "UPDATE ", this->getTableName(),
                " SET ", fieldNames[0],
...
{
    // 构建sql初始化
    std::string query = utils::concat(std::move(sql),
        this->buildOrderClause<Order, FieldName>(),
        this->buildLimitClause<Pagination>());
    MySQLStmtPtr stmtPtr = this->prepareStatement(query);
    ...
    const std::vector<std::string>& fieldNames) const
{
    ...
    // 构建查询语句
    std::string query = utils::concat(
        "SELECT ", utils::join_range(", ", fieldNames),
...
template<typename Entity, typename ID_TYPE>
inline  BaseMapper<Entity, ID_TYPE>::StatusWithAffectedRows BaseMapper<Entity, ID_TYPE>::deleteAll() const
{
    return MySQLConnectionPool::getInstance()->executeWithConnection(
        [this](MYSQL* mysql) -> StatusWithAffectedRows {
            if (mysql_query(mysql, utils::concat("TRUNCATE TABLE ", this->getTableName()).c_str())) {
    ...
    Args&&... args) const
{
    ...
    try {
        std::string query = utils::concat(
            "UPDATE ", this->getTableName(),
            " SET ", utils::join_range(", ", setClauses),
            " WHERE ", condition->buildConditionClause());
...
template<typename Entity, typename ID_TYPE>
MySQLStmtPtr BaseMapper<Entity, ID_TYPE>::prepareStatement(const std::string& query) const
{
    return MySQLConnectionPool::getInstance()->executeWithConnection(
...
template<typename Entity, typename ID_TYPE>
inline void BaseMapper<Entity, ID_TYPE>::executeQuery(MySQLStmtPtr& stmtPtr) const
{
    MYSQL_STMT* stmt = stmtPtr;
...
template<typename Entity, typename ID_TYPE>
template<typename... Args>
inline void BaseMapper<Entity, ID_TYPE>::bindValues(MySQLStmtPtr& stmtPtr, Args&&... args) const
{
    MYSQL_STMT* stmt = stmtPtr;
...
template<typename Entity, typename ID_TYPE>
template<typename... Args>
inline void BaseMapper<Entity, ID_TYPE>::bindValues(MySQLStmtPtr& stmtPtr, MYSQL_BIND* existingBinds, size_t& startIndex, Args&&... args) const
{
    MYSQL_STMT* stmt = stmtPtr;
    constexpr size_t count = sizeof...(Args);
    if (count == 0) return;
...
template<typename Entity, typename ID_TYPE>
inline void BaseMapper<Entity, ID_TYPE>::bindConditionValues(MySQLStmtPtr& stmtPtr, const std::unique_ptr<IQueryCondition>& condition) const
{
    size_t startIndex = 0;
...
template<typename Entity, typename ID_TYPE>
inline void BaseMapper<Entity, ID_TYPE>::bindConditionValues(MySQLStmtPtr& stmtPtr, const std::unique_ptr<IQueryCondition>& condition, MYSQL_BIND* existingBinds, size_t& startIndex) const
{
    MYSQL_STMT* stmt = stmtPtr;
...
inline constexpr auto BaseMapper<Entity, ID_TYPE>::buildOrderClause()
{
    ...
    return utils::concat(" ORDER BY ", FieldName, direction);
}
...
inline constexpr auto BaseMapper<Entity, ID_TYPE>::buildLimitClause()
{
    if constexpr (Pagination.limit == 0 && Pagination.offset == 0) {
        ...
        // 只有 OFFSET
        return utils::concat(" LIMIT ", std::to_string(MAX_ROW_SIZE_U64), " OFFSET ", std::to_string(Pagination.offset));
    }
    else {
        // LIMIT 和 OFFSET
        return utils::concat(" LIMIT ", std::to_string(Pagination.limit),
