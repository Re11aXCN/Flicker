...
class IQueryCondition {
 public:
    ...
    // 绑定参数到预处理语句
    virtual void bind(MySQLStmtPtr& stmtPtr, MYSQL_BIND* binds, size_t& bindIndex) const = 0;
...
public:
    ...
    void bind(MySQLStmtPtr& stmtPtr, MYSQL_BIND* binds, size_t& bindIndex) const override {
...
public:
    ...
    void bind(MySQLStmtPtr& stmtPtr, MYSQL_BIND* binds, size_t& bindIndex) const override {
        // 使用ValueBinder绑定参数
        ValueBinder::bindSingleValue(binds[bindIndex++], _value);
    }
...
public:
    ...
    void bind(MySQLStmtPtr& stmtPtr, MYSQL_BIND* binds, size_t& bindIndex) const override {
        // 绑定最小值和最大值
        ValueBinder::bindSingleValue(binds[bindIndex++], _minValue);
        ValueBinder::bindSingleValue(binds[bindIndex++], _maxValue);
    }
...
public:
    LikeCondition(const std::string& fieldName, const varchar& pattern)
        : _fieldName(fieldName), _pattern(pattern) {
    }
    LikeCondition(std::string&& fieldName, varchar&& pattern)
    ...
    void bind(MySQLStmtPtr& stmtPtr, MYSQL_BIND* binds, size_t& bindIndex) const override {
        // 绑定模式字符串
        ValueBinder::bindSingleValue(binds[bindIndex++], _pattern);
    }
...
public:
    RegexpCondition(const std::string& fieldName, const varchar& pattern)
        : _fieldName(fieldName), _pattern(pattern) {
    }
    RegexpCondition(std::string&& fieldName, varchar&& pattern)
        : _fieldName(std::move(fieldName)), _pattern(std::move(pattern)) {
    }
...
public:
    ...
    void bind(MySQLStmtPtr& stmtPtr, MYSQL_BIND* binds, size_t& bindIndex) const override {
        ...
        for (const auto& value : _values) {
            ValueBinder::bindSingleValue(binds[bindIndex++], value);
        }
    }
...
public:
    ...
    // 模糊匹配条件
    static std::unique_ptr<IQueryCondition> like_(const std::string& fieldName, const varchar& pattern) {
        return std::make_unique<LikeCondition>(fieldName, pattern);
    }
    
    // 正则表达式条件
    static std::unique_ptr<IQueryCondition> regexp_(const std::string& fieldName, const varchar& pattern) {
    ...
    static std::unique_ptr<IQueryCondition> like_(std::string&& fieldName, varchar&& pattern) {
        return std::make_unique<LikeCondition>(std::move(fieldName), std::move(pattern));
    }

    static std::unique_ptr<IQueryCondition> regexp_(std::string&& fieldName, varchar&& pattern) {
