...
SINGLETON_CREATE_CPP(ConfigManager)
ConfigManager::ConfigManager()
{
    std::string configPath;
    auto result = utils::get_executable_path();

    if (result) {
        auto configResult = utils::get_nth_parent_directory(result.value(), 3);

        std::string configDir = "";
        if (configResult) {
            configDir = configResult->string(); // 直接使用 path 的 string()
        }
        else {
            LOGGER_ERROR(configResult.error());
        }
        configPath = utils::concat(std::move(configDir), utils::local_separator(),
            "Common", utils::local_separator(),
            "config", utils::local_separator(),
            "config.json");
    }
    else
    {
        const auto& error = result.error();
        LOGGER_ERROR(std::format("Failed to get executable path, system error: {}, code: {}, message: {}", error.system_error.message(), magic_enum::enum_name(error.code), error.message));
    }
...
bool ConfigManager::loadFromFile(const std::string& filePath)
{
    ...
    try {
        if (!std::filesystem::exists(filePath)) {
            LOGGER_WARN(std::format("配置文件 {} 不存在，将使用默认配置初始化服务器", filePath));
            return false;
        }
        ...
        if (!Json::parseFromStream(builder, configFile, &root, &errs)) {
            LOGGER_ERROR(std::format("解析Json配置文件失败，错误原因: {}", errs));
            return false;
        }
        ...
        LOGGER_INFO(std::format("成功加载配置文件: {}", filePath));
        return true;
    }
    catch (const std::exception& e) {
        LOGGER_ERROR(std::format("加载配置文件异常: {}", e.what()));
        return false;
    }
}
...
bool ConfigManager::saveToFile(const std::string& filePath) const
{
    ...
    try {
        ...
        LOGGER_INFO(std::format("成功保存配置到文件: {}", filePath));
        return true;
    }
    catch (const std::exception& e) {
        LOGGER_ERROR(std::format("保存配置文件异常: {}", e.what()));
        return false;
    }
}