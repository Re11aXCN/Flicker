...
UserTestMapper::UserTestMapper() : BaseMapper<UserTestEntity, std::uint32_t>() {

}

constexpr std::string UserTestMapper::getTableName() const {
    return "test_users";
}

constexpr std::string UserTestMapper::findByIdQuery() const {
...
constexpr std::string UserTestMapper::findAllQuery() const {
    return "SELECT id, uuid, username, email, password, "
           "UNIX_TIMESTAMP(create_time) * 1000 as create_time_ms, "
           "UNIX_TIMESTAMP(update_time) * 1000 as update_time_ms "
...
constexpr std::string UserTestMapper::findByEmailQuery() const {
    return "SELECT id, uuid, username, email, password, "
        "UNIX_TIMESTAMP(create_time) * 1000 as create_time_ms, "
        "UNIX_TIMESTAMP(update_time) * 1000 as update_time_ms "
...
constexpr std::string UserTestMapper::findByUsernameQuery() const {
    return "SELECT id, uuid, username, email, password, "
        "UNIX_TIMESTAMP(create_time) * 1000 as create_time_ms, "
        "UNIX_TIMESTAMP(update_time) * 1000 as update_time_ms "
...
constexpr std::string UserTestMapper::insertQuery() const {
    return "INSERT INTO " + getTableName() + 
           " (username, email, password) "
           "VALUES (?, ?, ?)";
}

constexpr std::string UserTestMapper::deleteByIdQuery() const {
    return "DELETE FROM " + getTableName() + " WHERE id = ?";
}

void UserTestMapper::bindInsertParams(MySQLStmtPtr& stmtPtr, const UserTestEntity& entity) const {
...
UserTestEntity UserTestMapper::createEntityFromBinds(MYSQL_BIND* binds, MYSQL_FIELD* fields, unsigned long* lengths,
...
UserTestEntity UserTestMapper::createEntityFromRow(MYSQL_ROW row, MYSQL_FIELD* fields, unsigned long* lengths, size_t columnCount) const
{
    using tp = std::chrono::system_clock::time_point;
    if (columnCount < 7) {
...
std::optional<UserTestEntity> UserTestMapper::findByEmail(const std::string& email) {
    try {
        ...
        LOGGER_ERROR(std::format("findByEmail error: {}", e.what()));
        throw;
    }
}
...
std::optional<UserTestEntity> UserTestMapper::findByUsername(const std::string& username) {
    try {
        ...
        LOGGER_ERROR(std::format("findByUsername error: {}", e.what()));
        throw;
    }
}

void UserTestMapper::testTrueCondition()
{
    try {
...

        {
            ...
            LOGGER_INFO(std::format("TrueCondition测试 - queryEntitiesByCondition查询结果数量: {}", results1.size()));
            for (const auto& entity : results1) {
                LOGGER_INFO(std::format("  实体: {}", entity.toString()));
            }
        }
        {
            std::vector<std::string> fields{ "username", "email", "password" };
            auto results2 = queryFieldsByCondition<>(condition, fields);
            LOGGER_INFO(std::format("TrueCondition测试 - queryFieldsByCondition查询结果数量: {}", results2.size()));
            for (const auto& entity : results2) {
                for (const auto& field : entity) {
                    LOGGER_INFO(std::format("  字段: {} 值: {}", field.first, getFieldMapValueOrDefault(entity, field.first, std::string{})));
                }
            }
        }
        {
            auto results3 = countByCondition(condition);
            LOGGER_INFO(std::format("TrueCondition测试 - countByCondition查询结果数量: {}", results3));
        }
        {
        ...
        }
        {
            ...
            LOGGER_INFO(std::format("TrueCondition测试 - deleteByCondition影响行数: {}", results5.second));
        }
        {
            auto results6 = queryFieldsByCondition<>(condition, {"UNIX_TIMESTAMP(create_time) * 1000 as create_time_ms"});
            LOGGER_INFO(std::format("UNIX_TIMESTAMP 时间测试: {}", results6.size()));
            for (const auto& entity : results6) {
                for (const auto& field : entity) {
                    ...
                    LOGGER_INFO(std::format("字段: {} double值: {} time_point值: {}",
                        field.first,
                        value,
                        utils::time_point_to_str(tp)));
...
void UserTestMapper::testEqualCondition() {
    try {
        ...
        // 输出结果
        LOGGER_INFO(std::format("等值条件测试 - 查询结果数量: {}", results.size()));
...
void UserTestMapper::testNotEqualCondition() {
    try {
        ...
        // 输出结果
        LOGGER_INFO(std::format("不等值条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("不等值条件测试失败: {}", e.what()));
    }
}

// 测试范围条件
void UserTestMapper::testBetweenCondition() {
    try {
        ...
        // 输出结果
        LOGGER_INFO(std::format("范围条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("范围条件测试失败: {}", e.what()));
    }
}

// 测试大于条件
void UserTestMapper::testGreaterThanCondition() {
    try {
        // 创建大于条件 (id > 2)
        ...
        // 输出结果
        LOGGER_INFO(std::format("大于条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
        ...
        // 输出结果
        LOGGER_INFO(std::format("大于等于条件测试 - 查询结果数量: {}", resultsGE.size()));
        for (const auto& entity : resultsGE) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("大于条件测试失败: {}", e.what()));
    }
}

// 测试小于条件
void UserTestMapper::testLessThanCondition() {
    try {
        // 创建小于条件 (id < 3)
        ...
        // 输出结果
        LOGGER_INFO(std::format("小于条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
        ...
        // 输出结果
        LOGGER_INFO(std::format("小于等于条件测试 - 查询结果数量: {}", resultsLE.size()));
        for (const auto& entity : resultsLE) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("小于条件测试失败: {}", e.what()));
    }
}
...
void UserTestMapper::testLikeCondition() {
    try {
        ...
        // 输出结果
        LOGGER_INFO(std::format("模糊匹配条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("模糊匹配条件测试失败: {}", e.what()));
    }
}
...
void UserTestMapper::testRegexpCondition() {
    try {
        ...
        // 输出结果
        LOGGER_INFO(std::format("正则表达式条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("正则表达式条件测试失败: {}", e.what()));
    }
}

// 测试IN条件
void UserTestMapper::testInCondition() {
    try {
        // 创建IN条件 (id IN (1, 3, 5))
        ...
        // 输出结果
        LOGGER_INFO(std::format("IN条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("IN条件测试失败: {}", e.what()));
    }
}

// 测试NOT IN条件
void UserTestMapper::testNotInCondition() {
    try {
        ...
        // 输出结果
        LOGGER_INFO(std::format("NOT IN条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("NOT IN条件测试失败: {}", e.what()));
    }
}

// 测试IS NULL条件
void UserTestMapper::testIsNullCondition() {
    try {
        ...
        // 输出结果
        LOGGER_INFO(std::format("IS NULL条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
        ...
        // 输出结果
        LOGGER_INFO(std::format("IS NOT NULL条件测试 - 查询结果数量: {}", resultsNotNull.size()));
        for (const auto& entity : resultsNotNull) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("IS NULL条件测试失败: {}", e.what()));
    }
}

// 测试AND条件
void UserTestMapper::testAndCondition() {
    try {
        ...
        // 输出结果
        LOGGER_INFO(std::format("AND条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("AND条件测试失败: {}", e.what()));
    }
}

// 测试OR条件
void UserTestMapper::testOrCondition() {
    try {
        // 创建OR条件 (id = 1 OR id = 3)
        ...
        // 输出结果
        LOGGER_INFO(std::format("OR条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("OR条件测试失败: {}", e.what()));
    }
}

// 测试NOT条件
void UserTestMapper::testNotCondition() {
    try {
        // 创建NOT条件 (NOT (id < 3))
        ...
        // 输出结果
        LOGGER_INFO(std::format("NOT条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("NOT条件测试失败: {}", e.what()));
    }
}

// 测试原始SQL条件
void UserTestMapper::testRawCondition() {
    try {
        // 创建原始SQL条件 (id % 2 = 0)
        ...
        // 输出结果
        LOGGER_INFO(std::format("原始SQL条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("原始SQL条件测试失败: {}", e.what()));
    }
}

// 测试更新操作
void UserTestMapper::testUpdateByCondition() {
    try {
        // 创建条件 (id = 1)
        ...
        // 输出结果
        LOGGER_INFO(std::format("更新操作测试 - 状态: {}", magic_enum::enum_name(status)));
        
        // 查询更新后的实体
        auto entity = findById(1);
        if (entity) {
            LOGGER_INFO(std::format("  更新后的实体: {}", entity->toString()));
            LOGGER_INFO(std::format("  新密码: {}", entity->getPassword()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("更新操作测试失败: {}", e.what()));
    }
}

// 测试删除操作
void UserTestMapper::testDeleteByCondition() {
    try {
        // 创建条件 (id = 5)
        ...
        // 输出结果
        LOGGER_INFO(std::format("删除操作测试 - 状态: {}", magic_enum::enum_name(status)));
        
        // 验证删除结果
        auto entity = findById(5);
        LOGGER_INFO(std::format("  ID=5的实体是否存在: {}", entity.has_value() ? "是" : "否"));
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("删除操作测试失败: {}", e.what()));
    }
}

// 测试复杂查询条件
void UserTestMapper::testComplexCondition() {
    try {
        ...
        // 输出结果
        LOGGER_INFO(std::format("复杂条件测试 - 查询结果数量: {}", results.size()));
        for (const auto& entity : results) {
            LOGGER_INFO(std::format("  实体: {}", entity.toString()));
        }
    } catch (const std::exception& e) {
        LOGGER_ERROR(std::format("复杂条件测试失败: {}", e.what()));
    }
}
