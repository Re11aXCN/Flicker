...
bool Logger::initialize(const std::string& fileName, GeneratePolicy generatePolicy, bool truncate/* = false*/
...
std::string Logger::_createLogFlie(const std::string& fileDir, const std::string& fileName)
{
    std::string actualFilename;
    switch (_generatePolicy)
    {
    case SingleFile: {
        actualFilename = universal::utils::string::concat(fileDir, universal::utils::path::local_separator(), fileName, ".log.html");
        break;
    }
    case MultiplePerDay: {
        ...
        actualFilename = universal::utils::string::concat(fileDir, universal::utils::path::local_separator(), fileName, " - [", dateStr.str(), "].log.html");
        break;
    }
    case MultiplePerRun: {
        auto now = std::chrono::system_clock::now();
...
fs::path Logger::_getExecutablePath()
{
    // Windows 实现
#ifdef _WIN32
