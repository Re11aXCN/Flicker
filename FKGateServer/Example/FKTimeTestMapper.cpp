...
FKTimeTestMapper::FKTimeTestMapper() : FKBaseMapper<FKTimeTestEntity, std::uint32_t>() {

}

constexpr std::string FKTimeTestMapper::getTableName() const {
    return "test_time";
}

constexpr std::string FKTimeTestMapper::findByIdQuery() const {
...
constexpr std::string FKTimeTestMapper::findAllQuery() const {
    return "SELECT * FROM " + getTableName() + " ORDER BY id";
}

constexpr std::string FKTimeTestMapper::insertQuery() const {
    return "INSERT INTO " + getTableName() +
        " year, datetime, date, time "
        "VALUES (?, ?, ?, ?)";
}

constexpr std::string FKTimeTestMapper::deleteByIdQuery() const {
    return "DELETE FROM " + getTableName() + " WHERE id = ?";
}

void FKTimeTestMapper::bindInsertParams(MySQLStmtPtr& stmtPtr, const FKTimeTestEntity& entity) const {
    ...
    std::string time_str = FKUtils::time_point_to_str(entity._datetime);
...
FKTimeTestEntity FKTimeTestMapper::createEntityFromBinds(MYSQL_BIND* binds, MYSQL_FIELD* fields, unsigned long* lengths,
    char* isNulls, size_t columnCount) const
{
    using tp = std::chrono::system_clock::time_point;
    return FKTimeTestEntity{
...
FKTimeTestEntity FKTimeTestMapper::createEntityFromRow(MYSQL_ROW row, MYSQL_FIELD* fields, unsigned long* lengths, size_t columnCount) const
{
    using tp = std::chrono::system_clock::time_point;
    return FKTimeTestEntity{
