    ...
    if (_pSocket.is_open()) {
        try {
            ...
            if (ec && ec != boost::asio::error::not_connected) {
                FK_SERVER_ERROR(std::format("关闭socket错误: {}", ec.message()));
            }
            ...
            FK_SERVER_ERROR(std::format("关闭socket异常: {}", ex.what()));
        }
    }
            ...
            try {
                ...
                if (ec) {
                    if (ec == boost::beast::http::error::end_of_stream) {
                        FK_SERVER_INFO("客户端关闭连接");
                    } else if (ec == boost::asio::error::operation_aborted) {
                        FK_SERVER_INFO("读取操作被取消");
                    } else {
                        FK_SERVER_ERROR(std::format("HTTP读取错误: {}", ec.message()));
                    }
                ...
                FK_SERVER_INFO(std::format("收到HTTP请求: {} {}", 
                ...
                FK_SERVER_ERROR(std::format("处理HTTP请求异常: {}", ex.what()));
                ...
                FK_SERVER_ERROR("处理HTTP请求时发生未知异常");
    ...
    _pTimeout.async_wait(
        [self = shared_from_this()](boost::beast::error_code ec)
        {
            ...
            if (!ec)
            {
                ...
                // 如果不是因为取消操作导致的错误
                FK_SERVER_ERROR(std::format("超时检查错误: {}", ec.message()));
            }
        });
}
...
void FKHttpConnection::_writeResponse()
{
    ...
    _pResponse.set(boost::beast::http::field::date, FKUtils::get_gmtime());
    ...
    boost::beast::http::async_write(
        ...
        [self = shared_from_this()](boost::beast::error_code ec, std::size_t bytes_transferred)
        {
            ...
            // 处理写入错误
            if (ec) {
                FK_SERVER_ERROR(std::format("写入响应错误: {}", ec.message()));
                return;
            }
            
            FK_SERVER_INFO(std::format("响应已发送: {} 字节, 状态: {}", 
                bytes_transferred, 
            ...
            try {
                ...
                if (shutdown_ec && shutdown_ec != boost::asio::error::not_connected) {
                    FK_SERVER_ERROR(std::format("关闭发送端错误: {}", shutdown_ec.message()));
                }
            } catch (const std::exception& ex) {
                FK_SERVER_ERROR(std::format("关闭连接异常: {}", ex.what()));
            }
        });
}
...
void FKHttpConnection::_handleRequest()
{
    try {
        ...
        // 记录请求信息
        FK_SERVER_INFO(std::format("处理HTTP请求: {} {}", 
        ...
        case boost::beast::http::verb::get: {
            ...
            // 记录解析后的URL和参数数量
            FK_SERVER_INFO(std::format("解析URL: {}, 参数数量: {}", _pUrl, _pQueryParams.size()));
        ...
        case boost::beast::http::verb::post: {
            ...
            // 记录请求体大小
            FK_SERVER_INFO(std::format("POST请求: {}, 请求体大小: {}", _pUrl, _pRequest.body().size()));
        ...
        case boost::beast::http::verb::options: {
            ...
            FK_SERVER_INFO(std::format("处理OPTIONS请求: {}", _pRequest.target()));
        ...
        default:
            // 不支持的HTTP方法
            FK_SERVER_ERROR(std::format("不支持的HTTP方法: {}", _pRequest.method_string()));
        ...
        // 处理URL未找到的情况
        if (!success) {
            FK_SERVER_TRACE(std::format("URL未找到: {}", _pUrl));
        ...
        // 处理请求处理过程中的异常
        FK_SERVER_ERROR(std::format("处理请求异常: {}", ex.what()));
