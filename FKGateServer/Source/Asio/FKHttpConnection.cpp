			...
			try {
				...
				if (ec) {
					if (ec == boost::beast::http::error::end_of_stream) {
						...
						std::println("HTTP读取错误: {}", ec.message());
					}
					
					// 尝试优雅关闭连接
					try {
						self->_pSocket.shutdown(boost::asio::ip::tcp::socket::shutdown_both);
				...
				std::println("处理HTTP请求异常: {}", ex.what());
...
void FKHttpConnection::_checkTimeout()
{
	...
	_pTimeout.async_wait(
		[self = shared_from_this()](boost::beast::error_code ec)
		{
			...
			if (!ec)
			{
				std::println("连接超时，关闭socket");
				
				// 尝试关闭socket以取消所有未完成的操作
				try {
					boost::beast::error_code close_ec;
					self->_pSocket.close(close_ec);
					if (close_ec) {
						std::println("关闭socket错误: {}", close_ec.message());
					}
				} catch (const std::exception& ex) {
					std::println("关闭socket异常: {}", ex.what());
				}
			}
			else if (ec != boost::asio::error::operation_aborted) {
				// 如果不是因为取消操作导致的错误
				std::println("超时检查错误: {}", ec.message());
			}
		});
}
...
void FKHttpConnection::_writeResponse()
{
	...
	_pResponse.set(boost::beast::http::field::date, FKUtils::get_http_date());
	...
	boost::beast::http::async_write(
		...
		[self = shared_from_this()](boost::beast::error_code ec, std::size_t bytes_transferred)
		{
			...
			// 处理写入错误
			if (ec) {
				std::println("写入响应错误: {}", ec.message());
				return;
			}
			
			std::println("响应已发送: {} 字节, 状态: {}", 
				bytes_transferred, 
			...
			try {
				...
				if (shutdown_ec && shutdown_ec != boost::asio::error::not_connected) {
					std::println("关闭发送端错误: {}", shutdown_ec.message());
				}
			} catch (const std::exception& ex) {
				std::println("关闭连接异常: {}", ex.what());
			}
		});
}
...
void FKHttpConnection::_handleRequest()
{
	try {
		...
		case boost::beast::http::verb::get: {
			...
			// 记录解析后的URL和参数数量
			std::println("解析URL: {}, 参数数量: {}", _pUrl, _pQueryParams.size());
		...
		case boost::beast::http::verb::post: {
			...
			// 记录请求体大小
			std::println("POST请求: {}, 请求体大小: {}", 
				_pUrl, 
				_pRequest.body().size());
			
			// 调用业务逻辑处理
			success = FKLogicSystem::getInstance()->callBack(_pUrl, Http::RequestType::POST, shared_from_this());
			break;
		}
		case boost::beast::http::verb::options: {
			// 处理OPTIONS请求（CORS预检请求）
			std::println("处理OPTIONS请求: {}", _pRequest.target());
		...
		default:
			// 不支持的HTTP方法
			std::println("不支持的HTTP方法: {}", _pRequest.method_string());
