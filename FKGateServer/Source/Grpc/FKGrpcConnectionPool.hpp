...
public:
        ...
        : _pConfig(config)
    {
        ...
        try {
            ...
            FK_SERVER_INFO(std::format("grpc连接池初始化成功，服务地址: {}，连接池大小: {}", _pConfig.getAddress(), _pConfig.PoolSize));
        }
        catch (const std::exception& e) {
            FK_SERVER_ERROR(std::format("grpc连接池初始化失败: {}", e.what()));
            throw;
        }
    }
    ...
    std::unique_ptr<StubType> getConnection() {
        ...
        // 连接池为空，需要创建新连接
        if (!hasConnection) {
            FK_SERVER_WARN(std::format("获取连接超时，创建新连接 (活跃连接数: {})", _pActiveConnections.load()));
            // 检查是否超过最大连接数限制
    ...
    auto executeWithConnection(Func operation) -> decltype(operation(std::declval<StubType*>())) {
        ...
        try {
            ...
            FK_SERVER_ERROR(std::format("获取grpc连接失败: {}", e.what()));
            throw; // 重新抛出异常，让调用者处理
        }
        ...
        try {
            ...
            if (duration.count() > 100) { // 只记录耗时超过100ms的调用
                FK_SERVER_INFO(std::format("grpc调用耗时较长: {}ms", duration.count()));
            }
            ...
            FK_SERVER_ERROR(std::format("grpc操作执行异常: {}", e.what()));
            releaseConnection(std::move(connection));
            throw; // 重新抛出异常，让调用者处理
        }
        catch (...) {
            FK_SERVER_ERROR("grpc操作执行未知异常");
            releaseConnection(std::move(connection));
...
private:
    std::unique_ptr<StubType> _createStub() {
        try {
            ...
            FK_SERVER_ERROR(std::format("创建grpc连接失败: {}", e.what()));
            throw;
        }
    }
