...
bool FKUserMapper::createTableIfNotExists() {
    return _pConnectionPool->executeWithConnection([this](mysqlx::Session* session) {
        try {
            // 创建用户表SQL
            ...
            // 执行SQL
            session->sql(createTableSQL).execute();
...
bool FKUserMapper::insertUser(FKUserEntity& user) {
    return _pConnectionPool->executeWithConnection([&user, this](mysqlx::Session* session) {
        try {
            // 获取表对象
            auto schema = session->getSchema(""); // 使用当前会话的schema
...
std::optional<FKUserEntity> FKUserMapper::_findUserByCondition(const std::string& whereClause, 
    ...
    return _pConnectionPool->executeWithConnection([this, whereClause, paramName, paramValue](mysqlx::Session* session) {
        try {
            // 获取表对象
            mysqlx::abi2::r0::Schema schema = session->getSchema("");
            mysqlx::abi2::r0::Table table = schema.getTable(_tableName);

            // 查询用户 - 使用fetchOne而不是fetchAll，因为id、uuid、用户名和邮箱都是唯一的
            mysqlx::abi2::r0::RowResult result = table.select("*")
                .where(whereClause)
                .bind(paramName, paramValue)
                .execute();
            // 处理结果 - 直接使用fetchOne获取单条记录
            mysqlx::abi2::r0::Row row = result.fetchOne();
            if (!row) {
                return std::optional<FKUserEntity>();
            }
			return std::optional<FKUserEntity>(_buildUserFromRow(row, result.getColumns()));
        } catch (const std::exception& e) {
...
bool FKUserMapper::deleteUser(int id) {
    return _pConnectionPool->executeWithConnection([id, this](mysqlx::Session* session) {
        try {
            // 获取表对象
            auto schema = session->getSchema("");
            auto table = schema.getTable(_tableName);
    ...
    return _pConnectionPool->executeWithConnection([this](mysqlx::Session* session) {
        ...
        try {
            ...
            for (const auto& row : rows) {
                users.push_back(_buildUserFromRow(row, result.getColumns()));
            }
...
FKUserEntity FKUserMapper::_buildUserFromRow(const mysqlx::Row& row, const mysqlx::RowResult::Columns& columnsInfo){
    
    // 创建列名到索引的映射
    std::unordered_map<std::string, mysqlx::col_count_t> columnMap;
    for(auto it = columnsInfo.begin(); it != columnsInfo.end(); ++it) {
        columnMap[it->getColumnName()] = std::distance(columnsInfo.begin(), it);
        std::cout << "列名：" << *it << "数据： " << row[std::distance(columnsInfo.begin(), it)] << std::endl;
	}
    ...
    return FKUserEntity(id, uuid, username, email, password, salt);
