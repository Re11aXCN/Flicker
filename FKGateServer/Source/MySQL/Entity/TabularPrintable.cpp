...
std::string TabularPrintable::toString() const
 {
...
size_t TabularPrintable::_max(const std::vector<std::string>& vec, size_t(*getter)(const std::string&), const std::string& header) const
{
    size_t max = header.size();
    for (const auto& s : vec) {
        size_t len = getter(s);
